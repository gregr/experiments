      ...
      (let ((arg* (map value-value arg*)))
        ...
        (define (numpred2 ?)
          ...
          (let ((a (walk S (car  arg*)))
            ...
            (cond
              ...
              (else                            (suspend)))))
        ;; NOTE: we could redefine arithmetic to run backwards too.
        ;; If we do, we probably want to implement a "wait" operation, to allow us
        ;; to block until a variable becomes ground.  This way we can control
        ;; dataflow more tightly.  Then we can define wait+, wait*, etc.
        (define (arith3 +* valid*?)
          (argcount?! 3)
          (let ((a   (walk S (car   arg*)))
                (b   (walk S (cadr  arg*)))
        ...
        (define (arith2 +* valid*?)
          ...
          (let ((a (walk S (car  arg*)))
            ...
            (and (valid*? a b)
                 (cond
                   ((and (number? a) (number? b))     (return (+* a b)))
                   ((not (or (number? a)   (var? a))) #f)
                   ((not (or (number? b)   (var? b))) #f)
                   (else                              (suspend))))))
        (cond
          ...
          (else (case name
                  ...
                  ((-o) (arith3 - (lambda _               #t)))
                  ((*o) (arith3 * (lambda _               #t)))
                  ((/o) (arith3 / (lambda (num denom out) (not (eqv? denom 0)))))
                  ((+)  (arith2 + (lambda _           #t)))
                  ((-)  (arith2 - (lambda _           #t)))
                  ((*)  (arith2 * (lambda _           #t)))
                  ((/)  (arith2 / (lambda (num denom) (not (eqv? denom 0)))))
