...
-- labToLam_expr (CPSLam (txt, lab)) = M.insert lab txt (labToLam_lam txt) 
-- labToLam_expr _ = M.empty
-- labToLam_call ((proc, args), _) = unionmapM labToLam_expr (proc:args)
-- labToLam_lam (_, call) = labToLam_call call

-- progm lam = labToLam_expr $ CPSLam lam

type Label = [Int]

newLab = [0]
nextLab (minor:major) = (minor+1):major
subLab lab = 0:lab

type Var = String

type DLamTxt = ([Var], DirectTerm)
type DCallTxt = (DTerm, DTerm)
data DirectTerm = DirectRef Var
                | DirectLam DLamTxt
                | DirectApp DCallTxt
type DTerm = (DirectTerm, Label)

type Ref = (Var, Label)
type LamTxt = ([Var], Call)
type Lam = (LamTxt, Label)
type CallTxt = (CPSTerm, [CPSTerm])
type Call = (CallTxt, Label)
data CPSTerm = CPSRef Ref
             | CPSLam Lam
               deriving (Show, Ord, Eq)

directToCPS ((DirectRef var), lab) qont =
    ((qont, [CPSRef (var, lab)]), subLab lab)
directToCPS ((DirectLam (formals, body)), lab) qont = ((qont, [CPSLam (, )]), )
    where bodyCall = directToCPS body newK
          newK = CPSRef (, )
directToCPS ((DirectApp (proc, arg)), lab) qont = call
    where call = directToCPS proc ()
          call' = directToCPS arg ()
          call'' = ((u', [u'', qont]), )
          u' = ()
          u'' = ()
...
free_expr (CPSRef (var, _)) = S.insert var S.empty
free_expr (CPSLam lam) = free_lam lam
...
evalArg (CPSRef (var, _)) venv = mget venv $ var
evalArg (CPSLam lam) _ = S.singleton $ Clo lam
...
----------------------------------------------------------------
mkRef var lab = CPSRef (var, [lab])
mkLam formals call lab = CPSLam ((formals, call), [lab])
...
CPSLam example = (mkLam ["halt"] (mkLet "id" (mkLam ["x", "k"] (mkCall (mkRef "k" 0) [(mkRef "x" 1)] 7) 11)
                 (mkCall (mkRef "id" 2)
